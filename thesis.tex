...
The \mintinline{c}{vmspace} structure is the highest-level structure describing
the virtual address space of a process. It contains both the machine-independent
(\mintinline{c}{vm_map}) and machine-dependent (\mintinline{c}{pmap}) structures
...
\end{minted}
\caption{\texttt{vm/vm\_map.h}: Simplified definition of \mintinline{c}{struct vmspace}}
\end{listing}

\section{\mintinline{c}{vm_map}}

The \mintinline{c}{vm_map} structure represents the machine-independent part of
a virtual address space. It is structured as a tree of
\mintinline{c}{vm_map_entry} structures, each of which describes a continuous
...
\end{minted}
\caption{\texttt{vm/vm\_map.h}: Simplified definition of \mintinline{c}{struct vm_map}.}
\end{listing}

\subsection*{\mintinline{c}{struct vm_map_entry header;}}
This \mintinline{c}{vm_map_entry} is used for holding the minimum and maximum
virtual address available to the user. It also serves as the header of a linked
list of all \mintinline{c}{vm_map_entry} structures in the
\mintinline{c}{vm_map}, sorted by start address. It is used for quickly
retrieving the immediate left and right neighbours of a
\mintinline{c}{vm_map_entry}, as well as iterating over all entries in a
\mintinline{c}{vm_map}.
...
the value of \mintinline{c}{system_map} is \mintinline{c}{TRUE},
\mintinline{c}{system_mtx} is used exclusively, otherwise only
\mintinline{c}{lock} is used.
...
The \mintinline{c}{vm_map_entry} structure describes a continuous, page-aligned
...
\end{minted}
\caption{\texttt{vm/vm\_map.h}: Definition of \mintinline{c}{struct vm_map_entry}.}
\end{listing}
...
composed of 3 fields: \mintinline{c}{VM_PROT_READ},
\mintinline{c}{VM_PROT_WRITE}, and \mintinline{c}{VM_PROT_EXECUTE}.
...
\end{minted}
\caption{\texttt{vm/vm\_object.h}: Simplified definition of \mintinline{c}{struct vm_object}.}
\end{listing}

\subsection*{\mintinline{c}{vm_memattr_t memattr;}}

\mintinline{c}{memattr} specifies the default cache behaviour
\cite[Section~11.3]{bib:intel} of pages belonging to the object. For example,
contents of pages with the \mintinline{c}{VM_MEMATTR_UNCACHEABLE} attribute
...
\mintinline{c}{vm_object}. The \mintinline{c}{type} field determines the type of
...
mapping. Here is the definition of \mintinline{c}{struct md_page} for the x86
architecture:
...
The \mintinline{c}{pv_list} field is a list of structures representing virtual
...
\begin{itemize}
\item \mintinline{c}{PGA_REFERENCED}: An access has recently been made to the
  page.
\item \mintinline{c}{PG_FICTITIOUS}: The page doesn't represent a real physical
  page frame.
\end{itemize}
...
\end{minted}
\caption{\texttt{i386/include/pmap.h}: Simplified definition of \mintinline{c}{struct pmap}.}
\end{listing}
...
code for \mintinline{c}{do_execve} here.
...
\end{minted}
  \caption{\texttt{kern/imgact\_elf.c}: Simplified fragment of the
    \mintinline{c}{elf64_map_insert} function mapping a program segment into the
...
address space is the \mintinline{c}{vm_mmap_object} function. It does some basic
...
\bibitem{bib:splay} Daniel Dominic Sleator and Robert Endre Tarjan. 1985. \textit{Self-adjusting binary search trees}. J. ACM 32, 3 (July 1985), 652-686.
